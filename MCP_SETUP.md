...
Built into project — no setup. Requires Dart SDK 3.9+.

## Flame MCP

Community MCP server. Offline Flame docs + tutorials.
...
2. Sync Flame docs locally:

```bash
dart run bin/flame_doc_syncer.dart
```

> Avoid GitHub rate limits (60 req/hr) — set personal access token first:
...
`flame` server should show `✓ Connected`.