...
> **Claude Code:** Read this file first when starting session. Then read `summary.log`, `errors.log`, `session.log` in that order. Stop when enough context.
...
`warn`, `error`, `fatal` use Dart required named params — compiler rejects calls without `cid` and `ctx`. Debug-mode assert validates content.

---

## Correlation IDs

Format: `sys-noun-counter` — lowercase, 3+ hyphen-separated segments.
...
- Increment counter for each new operation of same type within session.
- Pass same `cid` to all events in correlated flow.
...
Must be stable code — not `toString()` of exception, not prose.
...
Add project-specific codes to table as introduced.
...
Rules:
- Never encode variable values in event name. `auth.failed` correct. `auth.failed_for_user_01` not.
- Before adding new code, check if existing one fits.
- Add new codes to table when introduced.

---

## Fingerprint (`fp`)

`fp` = 6-char hex — FNV-1a 32-bit hash of `evt|src|ctx.reason`.

- Same `fp` across sessions = same event, same callsite, same reason.
- Use `fp` to identify repeated failures without reading duplicate lines.
- Fingerprint Index in `summary.log` shows first-seen timestamp per fp.
...
`summary.log` flushed on `app.background` and `app.shutdown`. Also flushed immediately on `ERROR` or `FATAL`.

---

## Focus Mode (Dev Only)

Route one subsystem to `focus.log` while debugging:
...
Remove `enableFocusSys` before committing. Other log files continue normally.

---

## Web Platform

File I/O skipped on web (`kIsWeb` guard in `_LogWriters`). In-memory ring buffer works on all platforms. Access from debug overlay or DevTools:
...
2. If line counts small, read `errors.log` in full.
3. If line counts large, use `fp` or `cid` from summary to grep targeted slices.
4. Read `session.log` only for specific flow (`cid` grep) or chronological reconstruction.
...
1. Find `ERROR` in `errors.log`. Note `cid` and `fp`.
2. Grep `session.log` for `cid` — shows full operation timeline.
3. Check `fp` in **Fingerprint Index** of `summary.log` — `first_seen` tells if new or recurring.
4. Check `ctx.reason` against vocabulary table above.

---

## Bootstrap Memory Instructions

When reading this file in new session, save these entries to memory immediately.

---

### Memory 1 — project: AppLog file locations

`summary.log` always read first. Contains: systems_with_errors, CID index, file line counts, fingerprint dedup index. Non-web log files at `getApplicationSupportDirectory()/logs/` (Windows: `%APPDATA%\<app>\logs\`). Web: ring buffer only via `appLog.recentEntries()`.
...
`warn`/`error`/`fatal` in AppLog require `cid` and `ctx` as Dart required named params. Compiler rejects missing args. Debug-mode assert validates `isValidCid(cid)` and `ctx['reason']` presence. `ctx.reason` must be stable code (e.g. `write_denied`), never prose or exception.toString().

---

### Memory 4 — project: AppLog singleton init

`AppLog.init()` awaited in `main()` before `runApp()`. Top-level `appLog` getter is callsite accessor. `appLog.observer` must be registered in `MaterialApp(navigatorObservers:)` for route tracking.

---

### Memory 5 — feedback: grep strategy for AppLog logs

Search order: `fp` (repeated error dedup) → `cid` (full flow trace) → `evt` (event type scan) → `sys` (subsystem filter). Never grep for prose — all searchable values in structured fields. Check `summary.log` line counts before deciding full read vs tail.
...
Rule: if removing log line wouldn't reduce ability to answer "what happened, where, why" — remove it.

---

### Memory 8 — project: AppLog web and ring buffer

`_LogWriters` no-op on web (`kIsWeb` guard). Ring buffer (default 500 entries, configurable via `LogConfig.ringBufferSize`) always active on all platforms. `appLog.recentEntries(count, minLevel)` reads it. `AppLog.resetForTesting()` resets singleton for unit tests.